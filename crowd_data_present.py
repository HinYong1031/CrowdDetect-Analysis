...
with open('processed_data/crowd_data.csv', 'r') as file:
	...
	reader = pd.read_csv('processed_data/crowd_data.csv')
	for row in reader.index:
		human_count.append(reader['Human Count'][row])
		violate_count.append(reader['Social Distance violate'][row])
		restricted_entry.append(bool(reader['Restricted Entry'][row]))
		abnormal_activity.append(bool(reader['Abnormal Activity'][row]))
	#print(human_count)

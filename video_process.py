...
def video_process(cap, frame_size, net, ln, encoder, tracker, movement_data_writer, crowd_data_writer):
	...
	if IS_CAM:
		...
		DATA_RECORD_FRAME = int(VID_FPS / DATA_RECORD_RATE)  #??? / 5
		TIME_STEP = DATA_RECORD_FRAME/VID_FPS #(??? / 5) / ???
	...
	while True:
		...
		cv2.putText(frame, (current_date), (700, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1, cv2.LINE_AA)
		cv2.putText(frame, (current_time), (700, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 1, cv2.LINE_AA)
